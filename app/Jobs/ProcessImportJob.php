    ...
    public function handle(): void
    {
        ...
        foreach ($this->data as $row) {
            ...
            $dataInsert = [
                ...
                'base_faturamento_compra' => $row->data['BASE_x0020_FATURAMENTO_x0020_COMPRA'] ?? $this->convertToFloat($row->data['Base Faturamento Compra'] ?? 0.00),
                'valor_franquia' => $row->data['ValorFranquia'] ?? $this->convertToFloat($row->data['Valor Franquia'] ?? 0.00),
                'valor_total' => $row->data['Valor_x0020_Caixa'] ?? $this->convertToFloat($row->data['Valor Caixa'] ?? 0.00),
