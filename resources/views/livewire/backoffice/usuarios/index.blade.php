<div>
    ...
    <div class="flex items-center  mb-4 bg-base-100 p-4 rounded-lg shadow">
        <div class="w-1/3 ">
            <a href="{{ route('backoffice.usuarios.create') }}" class="bg-green-500 btn hover:bg-green-700">
                <x-icon name="s-plus" />
                Adicionar Usuário
            </a>
    ...
    <div class="w-full bg-white p-2 rounded-lg shadow overflow-hidden">
        <x-table :headers="$headers" :rows="$this->usuarios">
            ...
            <div class="flex items-center space-x-2">
                <a href="{{ route('backoffice.usuarios.edit', $usuario->id) }}"
                    class="text-white bg-blue-500 p-2 hover:bg-blue-700 btn-circle">
                    <x-icon name="s-pencil" />
                </a>
                <x-button icon="o-trash" wire:click.prevent="modalDelete('{{ $usuario->id }}')"
                    wire:key="delete-{{ $usuario->id }}" class="btn-circle bg-red-500 hover:bg-red-700 text-white" />
    ...
    <x-modal wire:model="modal_delete" title="Confirmar Exclusão" box-class="bg-red-200 border" class="backdrop-blur"
        ...
        <div class="flex flex-col items-center space-y-4">
            <span class="font-bold text-lg text-gray-700">Você tem certeza que deseja excluir este usuário?</span>
            <span class="font-black text-xl text-red-500">{{ $usuario_name }}</span>
        </div>
        <x-slot:actions>
            <x-button label="Cancel" @click="$wire.modal_delete = false" />
            <x-button label="Deletar" class="bg-red-500 hover:bg-red-700" wire:click="deleteUsuario" />
        </x-slot:actions>
    </x-modal>
