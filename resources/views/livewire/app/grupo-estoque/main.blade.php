<div>
    ...
    <x-header title="{{ $grupo_estoque->name }}" separator />
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 mb-4">
        <div class="bg-white rounded shadow p-4 mb-4 md:mb-0">
            ...
            @endphp
            <h3 class="text-lg font-semibold text-primary">Total de {{ $grupo_estoque->name }}</h3>
            <x-menu-separator />
        ...
        <div class="bg-white rounded shadow p-4 mb-4 md:mb-0">
            @php
                ...
                $percentual =
                    $quantidade == 0 ? 0 : (($quantidade - $quantidade_anterior) / $quantidade_anterior) * 100;
            @endphp
            <h3 class="text-lg font-semibold text-primary">Quantidade de {{ $grupo_estoque->name }}</h3>
            <x-menu-separator />
            <div class="flex gap-2 justify-center items-center">
                <div class="w-full p-2 h-full">
                    <div>
    ...
    <div class="flex flex-col gap-4">
        <div id="chart-quant" class="bg-white rounded shadow p-2">
            <livewire:app.grupo-estoque.chart-quant :grupo_estoque="$grupo_estoque" :data="$chartData" />
        </div>
        <div id="chart-filial" class="bg-white rounded shadow p-2">
            <livewire:app.grupo-estoque.chart-filial :grupo_estoque="$grupo_estoque" :data="$chartDataFilial" />
        </div>
